/**
 * Svelte store for managing archaeological site data
 * Using Svelte 4 stores pattern (compatible with Svelte 5)
 */
...
export const visibleSites = derived([sites, layerVisibility], ([$sites, $layerVisibility]) =>
		$sites.filter((site) => {
					switch (site.status) {
						case 'known':
...
export function toggleLayer(layer: keyof LayerVisibility) {
...
export function removeScanRegion(regionId: string) {
